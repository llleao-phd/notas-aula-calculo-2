...
A partir das operações sobre vetores, esta identificação sugere como \textit{somar} pares ordenados e como \textit{multiplicar um par ordenado por um escalar}, o que motiva a seguinte definição:
    \begin{definition}
        Sejam \((x,y)\) e \((s,t)\) dois elementos quaisquer de \(\R^2\) e \(\lambda\in\R\) qualquer.
            \begin{enumerate}
                \item A \textbf{soma}\index{Vetores! Soma} (adição) de \((x,y)\) com \((s,t)\) é dada por \[(x,y) + (s,t) = (x+s,y+t),\]
                \item A multiplicação por escalar é definida por \[\lambda \cdot (x,y) = (\lambda\cdot x,\lambda\cdot y),\] \index{Vetores! Multiplicação de um par ordenado por um escalar}
                \item A \textbf{diferença}\index{Vetores! Diferença} é \[(x,y) - (s,t) = (x,y) + (-1)(s,t),\]
...
\begin{definition}
    Definimos o \textbf{produto escalar}\index{Vetores! Produto escalar} entre dois vetores \((a_1,b_1)\) e \((a_2,b_2)\) por \[(a_1,b_1) \bullet (a_2,b_2) = a_1\cdot a_2 + b_1\cdot b_2.\]
\end{definition}
...
Para definirmos \textit{perpendicularismo} ou \textit{ortogonalismo} entre vetores de \(\R^2\) notamos que, para \(\overrightarrow{u} = (a_1,b_1)\) e \(\overrightarrow{v} = (a_2,b_2)\) aplicados no ponto \(P = (x,y)\in\R^2\), ou seja, podemos mostrar que (veja \cite{guidorizzi}[pg. 103]) \[\cos(\theta) = \frac{a_1a_2 + b_1b_2}{\sqrt{a_1^2 + b_1^2}\sqrt{a_2^2b_2^2}} = \frac{(a_1,b_1)\bullet(a_2,b_2)}{\sqrt{a_1^2 + b_1^2}\sqrt{a_2^2b_2^2}}.\]
    \begin{figure}[htbp]
            \centering
                \begin{tikzpicture}[x=0.55pt,y=0.55pt,yscale=-1,xscale=1]
                   %Straight Lines [id:da8958163654346065] 
                    ...
                    % Text Node
                    \draw (176.8,145.2) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize,color={rgb, 255:red, 208; green, 2; blue, 27 }  ,opacity=1 ]  {$\theta $};
                    % Text Node
...
\begin{definition}
    A \textbf{norma}\index{Vetores! Norma} de um vetor \((x,y)\) é definida por \[\|(x,y)\| = \sqrt{x^2 + y^2}\]
\end{definition}
...
\begin{theorem}[Desigualdade de Schwarz]\index{Teorema! Desigualdade de Schwarz}
        ...
        \begin{figure}[htbp]
            \centering
                \begin{tikzpicture}[x=1pt,y=1pt,yscale=-1,xscale=1]
                    ...
                    % Text Node
                    \draw (253.4,250.33) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$x$};
                    % Text Node
                    \draw (57.38,47.42) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$y$};
                    % Text Node
                    \draw (134.11,162.72) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$( x_{0} ,y_{0})$};
                    % Text Node
                    \draw (161.87,106.15) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$r$};
                    % Text Node
                    \draw (103.61,126.47) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$( x_{1} ,y_{1})$};
                    % Text Node
                    \draw (127.65,150.95) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize]  {$\alpha $};
            \end{tikzpicture}
